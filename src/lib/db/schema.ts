...
// `awaiting_human` and `human_active` split what used to be a single
// `escalated` state. A raised escalation with nobody attending yet is not the
// same as a teammate actively replying: in the first the bot keeps answering
// safe, factual questions so the traveller isn't left in silence (invariant
// 8), in the second it stays quiet so it never talks over a human.
